...
function injectComponentToHost(options: DependentConfig): Rule {
  return (host: Tree) => {
    const classifyName = `${strings.classify(
      options.hostName
    ...
    if (
      isImported(
        hostSource,
        classifyName,
        `./components/${strings.dasherize(
          options.hostName
        )}-${strings.dasherize(options.name)}.vue`
      )
    )
      return host
    ...
    const change = insertImport(
      ...
      hostFilePath,
      classifyName,
      `./components/${strings.dasherize(options.hostName)}-${strings.dasherize(
        options.name
      )}.vue`,
      true,
      25 // <script setup lang="ts">之后
    )
