  ...
  <Descriptions :class="className" :ref="$options.name" v-bind="binds">
    ...
    <template v-for="slots in computedSlots($slots)" #[slots.type]="scope" :key="slots.type">
      <template v-if="slots.type === 'default'">
        <DescriptionsItem v-for="(item, index) in items" :key="index" v-bind="item">
          <template v-for="slots in computedSlots($slots)" #[slots.type]="scope" :key="slots.type">
            <template v-if="slots.type === 'default'">
              <template v-if="!item.type || item.type === 'text'">
                <!-- 枚举 item -->
                <template
                  v-if="item.enumTypes && item.enumTypes.length"
                  >{{ (item.enumTypes.find((i: any) => i.value === attrs!.data![item.prop || '']) || {}).label }}</template
                >
                <!-- 时间 item -->
              ...
              <template v-else-if="item.type === 'slot'">
                <slot :name="item.slotName" v-bind="{ data: attrs!.data![item.prop || ''] }"></slot>
              </template>
            </template>
...
export default defineComponent({
  ...
  props: {
    c: {
      type: Object as PropType<DescriptionsAdapter>,
      default: () => reactive({}),
    },
    n: {
      type: String,
