...
import { HttpReturnType } from '@/api/http/src/http.dto'
import { FORM_VALIDATE_FAIL_ERROR, ASYNC_DATA_API_INVALID } from '@/core/utils/constants/errors'
import { ValidateResult } from '@/core/utils/dtos'
...
export const useForm = ({ attrs }: UseFormOpt) => {
  ...
  const registerForm = inject('formGroupFormRegister', () => ({})) as Function
  registerForm({
    id: instance.uid,
  ...
  const logoutForm = inject('formGroupFormLogout', () => ({})) as Function
  onUnmounted(() => {
  ...
  watch(
    ...
    (v) => {
      ...
      attrs.value.modelsOutput = attrs.value.modelsHandler
        ? attrs.value.modelsHandler(_.cloneDeep(v))
        : _.cloneDeep(v)
    },
